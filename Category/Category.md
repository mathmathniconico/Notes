...
__定義__ $\mathscr{C}$を圏、$T, I\in\mathscr{C}$を対象とする。以下を定める。

- 全ての対象$X\in\mathscr{C}$に対して唯一つの射$X\rightarrow T$が存在するとき、$T$を$\mathscr{C}$における **終対象** （terminal object）と呼ぶ。
- 全ての対象$X\in\mathscr{C}$に対して唯一つの射$I\rightarrow X$が存在するとき、$I$を$\mathscr{C}$における **始対象** （initial object）と呼ぶ。

> 終対象へは全ての対象からの矢印が集まっており、始対象からは全ての対象に向かって矢印が伸びている。
