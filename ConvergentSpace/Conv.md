...
フィルター$\mathscr{F}\subset 2^{X}$について$\mathscr{F}\in\lambda(x)$であることを$\mathscr{F}\rightarrow x$と書けば$\lambda$を明示せずに収束空間を定義できる。この意味で上記を満たす関係$\rightarrow$を **収束構造** （convergent structure）とも呼ぶ。（フィルター全体$\Phi(X)$について$\Phi(X)\times X$の部分集合としての「関係」である。）$\mathscr{F}\rightarrow x$のとき$\mathscr{F}$は$x$に収束するといい、$x$は$\mathscr{F}$の収束先や極限点ともいう。
